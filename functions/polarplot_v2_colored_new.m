function [dirrate] = polarplot_v2_colored_new(PosMtx,spkx,spky,ts,binSizeDir,correction,shape)
samplerate = 25;
...
%%
subplot(2,2,1);
line([0 0],[-(max(Rate)) max(Rate)],'linewidth',3,'color','k');
hold on
...
col = cc;
patch('Faces',f,'Vertices',v,'FaceVertexCData',col,'EdgeColor','interp','FaceColor','non','LineWidth',6,'LineSmoothing','off');
% camroll(correction);
...
%     title(strcat(peak_HD_2,'Hz, Vector length=',Vector_length_2),'fontsize',12);
%     set(gca,'DataAspectRatio',[1 1 1],'PlotBoxAspectRatio',[1 1 1]);
...
Angles = [0:binSizeDir:360-binSizeDir]';
% Angles = [0:binSizeDir:360]';

[posx,posy] = center_path(posgx,posgy,shape);
...
dirBins = Angles;
    for iBin = 1 : numel ( dirBins )
     binInd = [];
       ...
       hold on
       SpikesToPlot = find(DirbinInd == iBin);a = 100;
